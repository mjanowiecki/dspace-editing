...
comLink = baseURL+'/rest/communities/'+commID+'/collections'
collections = requests.get(comLink, headers=header, cookies=cookies,
                           verify=verify).json()
for j in range(0, len(collections)):
    ...
    if collectionID not in skippedCollections:
        ...
        items = ''
        while items != []:
            collLink = baseURL+'/rest/collections/'+collectID+'/items?limit=200&offset='+str(offset)
            items = requests.get(collLink, headers=header, cookies=cookies,
                                 verify=verify)
...
for number, itemID in enumerate(itemList):
    ...
    if changeRecord is True:
        ...
        provNote = '\''+addedKey+': '+addedValue+'\' was added through batch\
                   process on '+dt+'.'
        provNoteElement = {}
